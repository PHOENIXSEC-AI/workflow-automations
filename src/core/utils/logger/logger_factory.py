...
class LoggerFactory:
    ...
    def _setup_logfire_tracing():
        ...
        # Disable the default OpenAI tracing first
        ...
        logfire.configure(
            ...
            send_to_logfire=True,  # Set to False if sending to another OpenTelemetry backend
        )
        
        
        # Use exclude_commands parameter for PyMongoInstrumentor
        # Note: The parameter is exclude_commands, not excluded_operations
        # logfire.instrument_pymongo(
        #     custom_span_attributes={"env": "dev" if app_config.is_development() else 'prod'},
        #     # Only capture statements for non-excluded commands (like aggregate)
        #     capture_statement=True
        # )
        
        # Suppress MongoDB-related scopes to prevent excessive tracing
